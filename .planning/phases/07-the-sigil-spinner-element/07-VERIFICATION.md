...
| 3a | Idempotent re-render — same value re-set yields byte-identical innerHTML, proving a real re-render occurred | ✓ VERIFIED | RESOLVED after initial verification (commit `badb0cb`). The test now attaches a `MutationObserver` on the host's children before the same-value `setAttribute` and requires a childList mutation, alongside the byte-identical equality. Proven discriminating by injecting a same-value short-circuit into `attributeChangedCallback` and observing the assertion go RED — the original equality-only assertion stayed green under that same regression. Probe reverted; production code byte-unchanged |
...
| WRAP-03 | Attribute changes re-render correctly; multiple elements independent, no id collisions | ✓ SATISFIED | Both halves proven. "Changes to a *new* value re-render correctly" is oracle-backed. The narrower "same-value re-set is a genuine re-render, not a coincidental no-op" half was RESOLVED after initial verification (commit `badb0cb`) via a `MutationObserver` assertion, fail-first proven against an injected short-circuit |
